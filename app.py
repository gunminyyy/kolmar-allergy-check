...
with col1:
    src_files = st.file_uploader("1. 원본 파일들 업로드 (HP 여부 자동인식)", type=["xlsx"], accept_multiple_files=True)
with col2:
...
if src_files and res_files:
    ...
    for idx, (src_f, res_f) in enumerate(zip(src_files, res_files), 1):
        # ⭐ 파일명 기반 자동 모드 판별 (사용자님의 핵심 로직)
        file_mode = "HP" if "HP" in src_f.name.upper() else "CFF"
        
        with st.expander(f"📋 {idx}번 [{file_mode} 모드]: {src_f.name} ↔ {res_f.name}", expanded=True):
            try:
                ...
                # --- A. 원본 데이터 수집 (자동 판별된 file_mode 사용) ---
                if file_mode == "CFF":
                    src_p = str(ws_src['D7'].value or "N/A").strip()
                    src_d = str(ws_src['N9'].value or "N/A").split(' ')[0]
                    ...
                    src_p = str(ws_src['B10'].value or "N/A").strip()
                    src_d = str(ws_src['E10'].value or "N/A").split(' ')[0]
                ...
                # --- B. 최종본 데이터 수집 ---
                res_p = str(ws_res['B10'].value or "N/A").strip()
                res_d = str(ws_res['E10'].value or "N/A").split(' ')[0]
                for r in range(1, 400):
                ...
                st.download_button(f"📄 {idx}번 PDF 저장", pdf_bytes, f"Result_{idx}.pdf", "application/pdf", key=f"dl_{idx}")
                all_pdf_data.append({"name": f"Result_{idx}_{res_p}.pdf", "data": pdf_bytes})

                wb_src.close(); wb_res.close()
            except Exception as e:
                st.error(f"{idx}번 처리 중 오류: {e}")
